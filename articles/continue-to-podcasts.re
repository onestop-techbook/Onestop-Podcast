...
幸いながらにんげんがへたエフエム（以下にんげんがへた）ではネタが枯渇することなく1年に隔月スパンで安定的な配信ができています。メインパーソナリティとしてakazunomaとyyyknの2人で回していますが、主にアジェンダ（配信内容、話すネタ）を考えているのはakazunomaです。
...
基本的な構成は
・タイトルコール
・話すネタ2〜3本（イベントなどの事情により増えますが、基本は3つ、という方針にしています）
・締めの言葉
...
当初、キャッチコピーという要素は存在しておらず、他のエフエムさんも聞いた結果「こんにちは、Webディレクターのakazunomaです」という感じでサッパリ挨拶をする予定でした（その方がスマートでかっこいいのでは？という印象）
ただ、akazunomaはアイドルやVtuberが好きなので「みんなのハートをいただき〜ま○ゆ♡」とか「はいどーもぉ！キズ○アイです！」みたいな定番挨拶をやってみたいなという考えがありました（これはyyyknにも言ったことがなく、本誌での発言が初です）
...
公開収録の収録環境は本書の「Mac でパーソナリティ側でのみオンライン収録環境を作る」などを参考にしてください。普段からDiscordで収録をしているPodcasterの方は環境が変わらずに収録できると思います。もし普段Discordを使用していない場合はぜひ「すぱきゃす」の利用をご検討ください。「すぱきゃす」での公開収録は誰でも可能ですし、どんなイメージか知りたい場合はaozora.fmの公開収録にぜひ遊びに来てください。
...
もともとはオンラインゲームの宣伝ラジオをそのオンラインゲームの中で、仲間とともに聴取する文化を参考にはじめました。そのオンラインゲームでも土曜日の21時にとうる街の特定の場所に集まって、ワイワイ感想を言い合いながら一緒に聞きます。その雰囲気が非常に良いと思ったので何度か実施してみました。
...
//image[windows_play_sound_02][ステレオミキサー]

次にMacの場合ですが、本書の「Macでパーソナリティ側でのみオンライン収録環境を作る」を参考に設定してください。この設定のあとDiscordの音声設定で入力デバイスをSoundflower(64ch)に、出力デバイスをSoundflower(2ch)にするとスピーカーで再生している音声が再生されます。
